from api.transaction import upload_transaction

from libs.santizer import format_date
from libs.santizer import make_image_file_name
...
class UploadWindow:
    def __init__(self, 
                 id_token:str ,
                 branch_path:str='Home', 
                 branch_options:list = ['Home'], 
                 tree=None):
    ...
    def save_data(self):
        ...
        _branch = self.branch_dropdown.get()
        ...
        try:
            ...
            if not input_valid['status']:
                ...
                return
            res_upload = upload_transaction(
                id_token=self.id_token, 
                t_date=_date, branch=_branch, cashflow=_cashflow, description=_description, 
                file_path=self.file_path
            )
            if res_upload['status'] == False:
                messagebox.showinfo("Fail", res_upload['message'])
                print(f"...[ERROR] {res_upload['message']}")
                return

            messagebox.showinfo("Success", "Transaction data uploaded successfully")
