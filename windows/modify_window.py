...
from api.transaction import get_receipt_image, get_transaction_daily, modify_transaction
...
class ModifyWindow(tk.Tk):
    def __init__(self, id_token, branch, branch_options=['Home']):
        super().__init__()
    ...
    def get_transaction(self):
        ...
        for record in history:
            try:
                if record['receipt']:
                    image_base64 = get_receipt_image(self.id_token, record['receipt'])
                    if image_base64:
